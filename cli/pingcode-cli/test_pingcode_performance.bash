...
write_fixture_config() {
  command jq -n --argjson now "$EPOCHSECONDS" '{
    ...
    projects:[{id:"p1",name:"项目一",states:[{id:"s1",name:"状态一"}]}]
  }' > "$CONFIG_FILE"
}
...
curl() {
  ...
  elif [[ "$TEST_MODE" == refresh && "$args" == *'/v1/project/projects'* ]]; then
    printf '%s\n200' '{"total":2,"values":[{"id":"p1","name":"项目一"},{"id":"p2","name":"项目二"}]}'
  elif [[ "$TEST_MODE" == refresh && "$args" == *'/v1/project/work_item/states'* ]]; then
    printf 'states\n' >> "$TEST_DIR/states.log"
    if [[ "$args" == *'Authorization: Bearer old'* ]]; then
      printf '{}\n401'
    else
      [[ "$args" == *'Authorization: Bearer new'* ]]
      printf '%s\n200' '{"values":[{"id":"s1","name":"状态一"}]}'
    fi
...
for (( page=0; page<10; page++ )); do
  command jq -n --argjson page "$page" '{total:1000,values:[
...
"$CLI" bugs --project 项目一 --state 状态一 > "$TEST_DIR/bugs.json"
...
: > "$TEST_DIR/pages.log"
if "$CLI" bugs --project 项目一 --state 状态一 > /dev/null 2> "$TEST_DIR/error"; then
  printf 'expected duplicate-page failure\n' >&2; exit 1
fi
[[ "$(wc -l < "$TEST_DIR/pages.log")" -eq 2 ]]
[[ "$(<"$TEST_DIR/error")" == *'分页没有前进'* ]]

# A 401 is handled inside the first state-request command substitution; the
# next project's request must consume the replacement shared token snapshot.
export TEST_MODE=refresh
write_fixture_config
"$CLI" projects refresh > "$TEST_DIR/projects.json"
[[ "$(wc -l < "$TEST_DIR/refresh.log")" -eq 1 ]]
[[ "$(wc -l < "$TEST_DIR/states.log")" -eq 3 ]]
command jq -e 'length == 2 and all(.[]; .states[0].id == "s1")' "$TEST_DIR/projects.json" >/dev/null
...
zsh -fc '
  ...
  words=(bugs --project 项目一 --state "")
  [[ "$(_pingcode_states)" == 状态一 ]]
