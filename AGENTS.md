...
scripts/install.sh               # 安装脚本，软链 CLI 工具 + zsh 补全
```
...
- 不要在 bash 脚本中做交互式 TUI（raw 模式按键监听），macOS 下兼容性差
...
- CLI 工具只维护 zsh 补全文件 `_tool-name`
- install.sh 将 zsh 补全安装到 `~/.zsh/completions/`
...
| 改动范围 | 回归测试入口 |
| --- | --- |
| 安装脚本 | `bash scripts/test_install.bash` |
