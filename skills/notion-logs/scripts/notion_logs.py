...
CONFIG_PATH = Path("~/.config/notion-logs/config.json").expanduser()
NOTION_BASE_URL = "https://api.notion.com/v1"
DEFAULT_NOTION_VERSION = "2026-03-11"
USER_AGENT = "notion-logs/1.0 (my-ai-toolkit)"
ALLOWED_RATINGS = {"💎💎", "💎", "⭐⭐⭐", "⭐⭐", "⭐"}
...
class NotionLogsError(RuntimeError):
    pass
...
def load_config(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise NotionLogsError(
            f"配置不存在: {path}\n"
            "先运行: uv run SKILL_DIR/scripts/notion_logs.py config-template > ~/.config/notion-logs/config.json"
        )
    try:
        with path.open(encoding="utf-8") as f:
            data = json.load(f)
    except json.JSONDecodeError as exc:
        raise NotionLogsError(f"配置 JSON 无效: {path}: {exc}") from exc
    if not isinstance(data, dict):
        raise NotionLogsError(f"配置必须是 JSON object: {path}")
...
def get_token(config: dict[str, Any], required: bool = True) -> str:
    ...
    if required and not token:
        raise NotionLogsError(
            ...
            "或在 ~/.config/notion-logs/config.json 写入 notion_token。"
        )
    return token
...
def get_omdb_key(config: dict[str, Any]) -> str:
    ...
    if not key:
        raise NotionLogsError(
            ...
            "或在 ~/.config/notion-logs/config.json 写入 omdb_api_key。"
        )
    return key
...
def resolve_data_source(config: dict[str, Any], value: str) -> str:
    ...
    if not data_source_id:
        raise NotionLogsError("data source id 不能为空")
    ...
    for attempt in range(3):
        ...
        try:
            ...
            if exc.code == 429 and attempt < 2:
                ...
                time.sleep(delay)
                continue
            raise NotionLogsError(f"HTTP {exc.code} {method} {url}\n{raw}") from exc
        except urllib.error.URLError as exc:
            raise NotionLogsError(f"请求失败 {method} {url}: {exc}") from exc
    raise NotionLogsError(f"请求重试后仍失败: {method} {url}")
...
def fetch_musicbrainz_album(album_name: str, artist_name: str) -> dict[str, Any]:
    ...
    if not release_groups:
        raise NotionLogsError(f"MusicBrainz 未找到专辑: {album_name} / {artist_name}")
...
def fetch_omdb(title: str, media_type: str, api_key: str) -> dict[str, Any]:
    ...
    if data.get("Response") == "False":
        raise NotionLogsError(f"OMDB 未找到: {title} (type={media_type}) - {data.get('Error', '')}")
...
def build_media_metadata(args: argparse.Namespace, config: dict[str, Any]) -> dict[str, Any]:
    if args.metadata_json:
        try:
            ...
            raise NotionLogsError(f"--metadata-json 不是有效 JSON: {exc}") from exc
        if not isinstance(data, dict):
            raise NotionLogsError("--metadata-json 必须是 JSON object")
        return data
    ...
    if args.metadata_file:
        with Path(args.metadata_file).expanduser().open(encoding="utf-8") as f:
            data = json.load(f)
        if not isinstance(data, dict):
            raise NotionLogsError("--metadata-file 必须包含 JSON object")
        return data

    if args.kind == "music":
        if not args.artist:
            raise NotionLogsError("记录 Music Logs 必须提供 --artist")
...
def build_media_properties(
    ...
    if rating not in ALLOWED_RATINGS:
        ...
        raise NotionLogsError(f"Rating 必须是已有 Notion 选项之一: {allowed}")
    ...
    if not title:
        raise NotionLogsError("metadata 中缺少标题，无法写入 Notion title 字段")
...
def command_media(args: argparse.Namespace) -> int:
    ...
    if not isinstance(media_config, dict):
        raise NotionLogsError(f"配置缺少 media.{args.kind}")
    property_map = media_config.get("properties", {})
    if not isinstance(property_map, dict):
        raise NotionLogsError(f"配置 media.{args.kind}.properties 必须是 object")
    ...
    if args.mode == "upsert":
        ...
        if len(matches) > 1:
            ...
            raise NotionLogsError(f"匹配到多个 Notion 条目，请改用 --page-id 指定: {urls}")
        if matches:
    ...
    if action == "update":
        if not page_id:
            raise NotionLogsError("--mode update 必须提供 --page-id，或使用 --mode upsert 自动查找")
        ...
        raise NotionLogsError(f"未知 action: {action}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config", default=str(CONFIG_PATH), help="配置文件路径，默认 ~/.config/notion-logs/config.json")
