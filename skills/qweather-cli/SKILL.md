...
使用 `qweather-cli` 完成天气查询。默认直接执行查询命令；只有用户明确要排查配置，或查询因配置问题失败时，再使用 `config` 相关命令。
...
- 不要因为 skill 里出现了 `YOUR_API_HOST`、`YOUR_API_KEY` 这样的示例值，就自动执行 `init`
- 如果当前机器还没初始化，先让用户提供真实的 `api_host` 和 `api_key`，再执行 `qweather-cli init ...`
- 如果用户没有给值，就只提示初始化方式，不要替用户猜测或写入占位值

若只是想确认当前 CLI 读到的配置，运行：
...
正常查询时不要额外先做一轮手工检查；直接执行 `now`、`daily` 或 `hourly`。只有当用户主动要求排查，或查询失败且看起来像配置问题时，再跑 `config check`。
...
- 用户要确认配置是否已初始化、当前 Host 是否生效、配置文件放在哪里，或快速检查配置是否可用时，用 `config show`、`config path` 或 `config check`
